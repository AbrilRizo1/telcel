(function(){
    ...
    function spawnCookie(){
        ...
        const which = Math.floor(rand(1,4));
    ...
    window.addEventListener('touchmove', e=>{ const t=e.touches[0]; if(t){ const r=canvas.getBoundingClientRect(); pointerX=t.clientX-r.left; } }, {passive:false});
    window.addEventListener('touchstart', e=>{ const t=e.touches[0]; if(t){ const r=canvas.getBoundingClientRect(); pointerX=t.clientX-r.left; } }, {passive:false});
    ...
    startBtn.style.display = 'none'; // ocultamos al inicio
    ...
    function startGame(){
        ...
        resize(); adjustBasketSize();
        for(let i=0;i<3;i++) spawnCookie();
        requestAnimationFrame(loop);
    }
    
    function resetGame(){
        running = false; score = 0; lives = LIFE_COUNT; timeLeft = START_TIME;
        cookies.length = 0; updateHUD(); clearScreen();
    }
    ...
    function loop(ts){
        ...
        for(let i=cookies.length-1;i>=0;i--){
            const c = cookies[i];
            c.y += c.vy + dt*60*c.vy*0.12;
            if(c.y+c.size/2 >= basket.y){
                if(c.x > basket.x && c.x < basket.x+basket.w){ score += POINTS_PER_COOKIE; }
                else { lives -= 1; }
                cookies.splice(i,1);
    ...
    levelModal.querySelectorAll('button').forEach(btn=>{
        btn.addEventListener('click', ()=>{
            ...
            // Mostramos botón de inicio solo al cerrar modal

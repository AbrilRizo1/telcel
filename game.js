(function(){
    ...
    function startGame(){ if(running) return; score=0;lives=LIFE_COUNT;timeLeft=START_TIME;cookies.length=0;running=true;lastTime=performance.now();resize();adjustBasketSize(); for(let i=0;i<3;i++) spawnCookie(); requestAnimationFrame(loop); }
    ...
    function loop(ts){
      ...
      for(let i=cookies.length-1;i>=0;i--){
        ...
        if(c.y+c.size/2>=basket.y){ const cx=c.x; if(cx>basket.x && cx<basket.x+basket.w){ score+=POINTS_PER_COOKIE; } else { lives-=1; } cookies.splice(i,1); if(lives<=0){ running=false; showEnd('Juego terminado'); return; } continue; }

(function(){
    ...
    const imgSources=['basket.png','cookie1.png','cookie2.png','cookie3.png'];
    const images={};
    imgSources.forEach(src=>{ const img=new Image(); img.src=src; img.onload=()=>{}; img.onerror=()=>{}; images[src]=img; });
    
    function rand(min,max){ return Math.random()*(max-min)+min; }
    function spawnCookie(){ if(W<=0) return; const size=Math.round(rand(36,78)); const x=rand(size/2,W-size/2); const speed=rand(1.2,4.5)*COOKIE_SPEED; const which=Math.floor(rand(1,4)); const img=images['cookie'+which+'.png']; cookies.push({x,y:-size,vy:speed,size,img}); }
    
    let pointerX=null,left=false,right=false;
    let tiltX=null;
    ...
    // Inclinación móvil suave
    window.addEventListener('deviceorientation', e=>{
      if(e.gamma!==null){
        const sensitivity=0.7;
        tiltX = W/2 + (e.gamma/45)* (W/2) * sensitivity;
      }
    });
    ...
    if(startBtn) startBtn.addEventListener('click',()=>{ if(!running) startGame(); });
    if(resetBtn) resetBtn.addEventListener('click', resetGame);
    
    function startGame(){
      if(running) return;
      score=0;lives=LIFE_COUNT;timeLeft=START_TIME;cookies.length=0;running=true;lastTime=performance.now();
      resize(); adjustBasketSize(); for(let i=0;i<3;i++) spawnCookie();
      requestAnimationFrame(loop);
    }
    function resetGame(){ running=false; score=0; lives=LIFE_COUNT; timeLeft=START_TIME; cookies.length=0; updateHUD(); clearScreen(); }
    ...
    function loop(ts){
      ...
      if(pointerX!==null){ basket.x+=(pointerX-(basket.x+basket.w/2))*0.25; }
      else { if(left) basket.x-=basket.speed; if(right) basket.x+=basket.speed; }
      basket.x=Math.max(8,Math.min(W-basket.w-8,basket.x));
    
      for(let i=cookies.length-1;i>=0;i--){
        const c=cookies[i]; c.y+=c.vy+dt*60*c.vy*0.12;
        if(c.y+c.size/2>=basket.y){
          const cx=c.x; if(cx>basket.x && cx<basket.x+basket.w){ score+=POINTS_PER_COOKIE; } else { lives-=1; }
          cookies.splice(i,1);
      ...
      for(const c of cookies){ if(c.img && c.img.complete){ ctx.drawImage(c.img,c.x-c.size/2,c.y-c.size/2,c.size,c.size); } else { ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(c.x,c.y,c.size/2,0,Math.PI*2); ctx.fill(); } }
      const basketImg=images['basket.png']; if(basketImg && basketImg.complete){ ctx.drawImage(basketImg,basket.x,basket.y,basket.w,basket.h); } else { ctx.fillStyle='#5a2'; ctx.fillRect(basket.x,basket.y,basket.w,basket.h); }
    ...
    function showEnd(text){
      ...
      if(/gan/i.test(text)){ setTimeout(()=>{ window.location.href='https://facebook.com'; },2500); }
    }
    ...
    levelModal.querySelectorAll('button').forEach(btn=>{
      btn.addEventListener('click',()=>{
        ...
        MAX_COOKIES=parseInt(btn.dataset.max);
